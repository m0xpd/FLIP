...
When the probability is set to p = 0.5, the process is equivalent to a "coin toss"; the clock is routed to output A or B based on the outcome of a fair coin toss at each clock cycle. As p moves from the 0.5 value, the coin becomes increasingly "biased", with the outcome favouring one or other output. Larger values of p favors output A.

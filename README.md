...
The system is builta around a trivial state machine which has two states, held on a SR flip-flop. I imlemented this on 4001 (NOR) and 4081 (AND) gates, which were to hand (Flip was designed around 'available materials'). The transitions for the state machine were sufficient to define the Set and Reset signals:

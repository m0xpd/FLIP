...
The stripboard FLIP layout is really too big for Eurorack (it certainly isn't skiff-friendly!) and would need a deep case to run in a Eurorack system. Most of my card frames are only 155mm deep, which isn't really enough to hold the stripboard (which is ~140mm long). I have one 6U frame which is about 175mm deep, but I don't plan to run the stripboard prototype *inside* my Eurorack system. I suggest anybody wishing to use FLIP in a Eurorack system should use the [dedicated design](https://github.com/m0xpd/Flip/blob/main/Eurorack/README.md).
